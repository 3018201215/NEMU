...
make_helper(concat(lods_, SUFFIX)) {
	if(ops_decoded.is_operand_size_16){
			if(DATA_BYTE == 1){
				REG(R_AL) = swaddr_read(reg_w(R_SI), DATA_BYTE);
				if(cpu.DF == 0){
		...
		if(DATA_BYTE == 1){
				reg_l(R_AL) = swaddr_read(reg_w(R_SI), DATA_BYTE);
				if(cpu.DF == 0){
	...
	print_asm("lods" str(SUFFIX) "0x%X 0x%X",swaddr_read(reg_l(R_ESI), DATA_BYTE),reg_l(R_AL));
